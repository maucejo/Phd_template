...
#let thesistemplate(
  ...
  show ref: it => {
    ...
    if el != none and el.func() == figure and el.kind == "subfigure" {
        context{
          let heading-counter = numbering(num-pattern.get(), counter(heading).get().first())
          let fig-counter = counter(figure.where(kind: image)).at(here()).first() + 1
          let subfig-numbering = numbering("a", counter(figure.where(kind: "subfigure")).at(el.location()).first())
          link(it.target, [#fig-supplement #heading-counter\.#fig-counter#subfig-numbering])
        }
    } else {
      it
    }
